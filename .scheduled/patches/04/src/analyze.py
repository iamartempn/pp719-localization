...
def tnved_coverage(df: pl.DataFrame, n: int = 20) -> pl.DataFrame:
    """
    Агрегирует данные по первым 4 символам кода ТН ВЭД.

    Возвращает топ-N категорий по числу позиций.
    """
    if "тн_вэд" not in df.columns:
        return pl.DataFrame({
            "тнвэд_префикс": [], "количество_позиций": [], "ср_баллы": []
        })

    return (
        df.filter(pl.col("тн_вэд").is_not_null())
        .with_columns(
            pl.col("тн_вэд").str.slice(0, 4).alias("тнвэд_префикс")
        )
        .group_by("тнвэд_префикс")
        .agg(
            pl.len().alias("количество_позиций"),
            pl.col("баллы").mean().alias("ср_баллы"),
        )
...
def main() -> None:
    ...
    print("\n--- Топ-20 категорий ТН ВЭД ---")
    print(tnved_coverage(df))
